...
void motion_reset()
{
    for(int i=0; i<4; i++){
        lowCmd.motorCmd[i*3+0].mode = 0x0A;
        lowCmd.motorCmd[i*3+0].positionStiffness = 200;
        lowCmd.motorCmd[i*3+0].velocity = 0;
        lowCmd.motorCmd[i*3+0].velocityStiffness = 2;
        lowCmd.motorCmd[i*3+0].torque = 0;
        lowCmd.motorCmd[i*3+1].mode = 0x0A;
        lowCmd.motorCmd[i*3+1].positionStiffness = 200;
        lowCmd.motorCmd[i*3+1].velocity = 0;
        lowCmd.motorCmd[i*3+1].velocityStiffness = 2;
        lowCmd.motorCmd[i*3+1].torque = 0;
        lowCmd.motorCmd[i*3+2].mode = 0x0A;
        lowCmd.motorCmd[i*3+2].positionStiffness = 300;
        lowCmd.motorCmd[i*3+2].velocity = 0;
        lowCmd.motorCmd[i*3+2].velocityStiffness = 3;
        lowCmd.motorCmd[i*3+2].torque = 0;
    }

    double pos[12] = {-0.0, 1.256, -2.355, -0.0, 1.256, -2.355, 
                      0.0, 1.256, -2.355, -0.0, 1.256, -2.355};
...
void motion_reset_ground()
{
    ...
    double pos[12] = {-0.0, 1.7, -2.6, -0.0, 1.7, -2.6, 
                       0.0, 1.7, -2.6, -0.0, 1.7, -2.6};
    moveAllPosition(pos, 5000);
