...
{
  //std::cout << "setIterations -------------------------------------------------" << std::endl;
  //std::cout << "iterationsPerMPC " << iterationsPerMPC << " currentIteration " << currentIteration << std::endl;
  ...
  // test
  //_phase = (double)(currentIteration % (iterationsPerMPC)) / (double) (iterationsPerMPC);
  //std::cout << "_iteration " << _iteration << " _phase " << _phase << std::endl;
  //std::cout << "progress " << getSwingSubPhase() << std::endl;
  //std::cout << "mpc table " << std::endl;
  //auto my_table = mpc_gait();
  // std::cout << *my_table << std::endl;
  // std::cout << my_table << std::endl;
  // for (int i=0; i < 40 ; i++){
  //   std::cout << my_table[i] << std::endl;
  // }
  //std::cout << "---------------------------------------------------------------" << std::endl;
}
...
void ConvexMPCLocomotion::setRLFootPositions(double feetPos[8])
{
  for( int i = 0; i < 4; i++){
    rlFeetPosx[i] = feetPos[2*i];
    rlFeetPosy[i] = feetPos[2*i+1];
  }
  useRLPos = true;
}
 ...
 //Integral-esque pitch and roll compensation
  ...
  for(int i = 0; i < 4; i++) {
   ...
   // pFoot[i] = data._legController->data[i].p;
    //std::cout << "pFoot " << i << " " << pFoot[i] << " newData? " << data._legController->data[i].p << std::endl;
    //std::cout << " newData? " << data._legController->data[i].p[0] << " " << data._legController->data[i].p[1] << " " << data._legController->data[i].p[2] << std::endl;
  }
  ...
  //std::cout << "Swing Times " << swingTimes << std::endl;
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(firstSwing[i]) {
      ...
      //Vec3<double> offset(0, side_sign[i] * .085, 0); // was .083
      ...
      double p_rel_max, pfx_rel, pfy_rel;

      // if(!useRLPos){
        p_rel_max = 0.4;
        pfx_rel = seResult.vWorld[0] * .5 * gait->_stance * dtMPC +
                        .03*(seResult.vWorld[0]-v_des_world[0]) +
                        (0.5*seResult.position[2]/9.81) * (seResult.vWorld[1]*stateCommand->data.stateDes[11]);
        pfy_rel = seResult.vWorld[1] * .5 * gait->_stance * dtMPC +
      ...
      // } else {
      if(useRLPos){  
        //have RL flag in this class to choose between, check relative pos vs abs, which frame are they in. 
        //may have to also modify F, along with the locations
        // should these have max/min like above? place that outside if?
        pfx_rel += rlFeetPosx[i];
        pfy_rel += rlFeetPosy[i];
        //std::cout << " set rl " << " pfx_rel: "  << pfx_rel << " pfy_rel "  << pfy_rel << std::endl;
      }
      Pf[0] +=  pfx_rel;
      Pf[1] +=  pfy_rel + interleave_y[i] * v_abs * interleave_gain;
      //Pf[2] = -0.01;
      Pf[2] = 0;// pFoot[i][2];
      //std::cout <<  i << " pfx_rel: "  << pfx_rel << " pfy_rel "  << pfy_rel << std::endl;
      //std::cout << "Final foot pos " << i << ": " << Pf[0] << " "  << Pf[1] << " "  << Pf[2] << std::endl;
  ...
  // load LCM leg swing gains (z gain is 350 in newer code)
  Kp << 500, 0, 0,
      0, 500, 0,
      0, 0, 350; //100
  Kp_stance = 0*Kp;
...
//  StateEstimator* se = hw_i->state_estimator;
  ...
  for(int foot = 0; foot < 4; foot++)
  {
    ...
    if (debugFlag)
      std::cout << "swing state " << foot << ": " << swingState << " swingTime " << swingTimes[foot] << std::endl;
    if(swingState > 0) // foot is in swing
    {
      ...
      // std::cout << "in SwingState swing " << foot << ": " << swingState << std::endl;
      // footSwingTrajectories[foot].computeSwingTrajectoryBezier(swingState, swingTimes[foot]);

      // test different phase offsets
      double dphase = swingState;// + 0.002 / swingTimes[foot]; //0.01/0.15;
      dphase = min(dphase,1.0); // clip to make sure not over 1
      footSwingTrajectories[foot].computeSwingTrajectoryBezier(dphase, swingTimes[foot]);
      ...
      //cout << "Foot " << foot << " relative velocity desired: " << vDesLeg.transpose() << "\n";
       ...
       // std::cout << "contat " << foot << ": " << contactState << std::endl;
        if (debugFlag){
          std::cout << "foot Des world " << foot << ": " << pDesLeg(0) << " " << pDesLeg(1) << " " << pDesLeg(2) << std::endl; 
    ...
    else if(contactState > 0) // foot is in stance
    {
      firstSwing[foot] = true;
       ...
       // test different phase offsets
      double dphase = swingState;// + 0.001 / swingTimes[foot]; //0.01/0.15;
      //dphase = min(dphase,1.0); // clip to make sure not over 1
      footSwingTrajectories[foot].computeSwingTrajectoryBezier(dphase, swingTimes[foot]);
      ...
      se_contactState[foot] = contactState;//1.0;
    ...
    // else
     //{
      ...
      double trajInitial[12] = {rpy_comp[0],  // 0
                                ...
                                xStart,                                   // 3
                                yStart,                                   // 4
                                0.3,      // 5 for Aliengo robot, change to 0.4, was 0.3
                                0,                                        // 6
                                0,                                        // 7
