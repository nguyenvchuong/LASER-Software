...
%% Load experiment RL data with MDC_power limit
M = dlmread('data_log_from_RL_exp/012623125238/jump_full_state.txt');
save('jump_full_state_case.mat','M');
% Indexing of experiment data
% Actual Joint velocity FR-25,26,27; FL-28,29,30; RR-31,32,33;RL-34,35,36.
% Cmd Joint torque: FR-93,94,95; FL-96,97,98; RR-99,100,101; RL-102,103,104.
% Get data from 1 to 1200ms, but every 10ms
actual_CoM = M(1:10:1200,1:12); % px, py, pz, r, p, y, vx, vy, vz, wx, wy, wz
actual_q = M(1:10:1200, 13:24);
actual_dq = M(1:10:1200,25:36);
actual_torque = M(1:10:1200,37:48);
cmd_torque = M(1:10:1200,93:104);
% get data for thigh and calf respectively
actual_q_2D = [actual_q(:,2),actual_q(:,3),actual_q(:,8),actual_q(:,9)];
actual_dq_2D = [actual_dq(:,2),actual_dq(:,3),actual_dq(:,8),actual_dq(:,9)];
cmd_torque_2D = [cmd_torque(:,2), cmd_torque(:,3), cmd_torque(:,8), cmd_torque(:,9)];
%%
...
option.use_voltage_constraint= 0;
...
option.file_name='jumpingFull_A1_10ms_h20_d60';
...
option.alpha_p=1; % weight in cost function for power
...
%     power=0;
%     for i=1:4
%         power = power+ 0.5*tau_joint(i,k)*dq(3+i,k)+ (0.5*tau_joint(i,k))^2*R_motor/(Kt*gear_ratio)^2;
%     end
%     J = J + option.alpha_p*power^2;
% end
...
%             opti.subject_to(tau_joint(:,k) >= -2*option.torque_saturation_stance);
    ...
    if option.use_upper_power_constraint
            ...
            opti.subject_to(power <= 0.615*option.voltage_max*option.current_max); % no charge back to the battery + power limits
    ...
    % joint angle limit    
    qj_min = option.q_joint_limit_lower; 
    qj_max = option.q_joint_limit_upper; 
...
% opti.set_initial(q(1:3,:),actual_CoM(:,1:3)');
% opti.set_initial(q(4:7,:),actual_q_2D');
opti.set_initial(tau_joint,repmat(2*ones(4,1),1,N));
% opti.set_initial(tau_joint,cmd_torque_2D');
